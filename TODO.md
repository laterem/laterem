...
- [ ] Категории работ: Категория предмета, подкатегория темы, объект работы. `Свободно`
    - [ ] Страницы/__панель__ навигации соответствующие категории. Отдельная страница для работы `Свободно`

***
## Глобальные задания

- [ ] Расхардкодить button1 `Первому кто догадается как это чёрт возьми делать`
- [ ] Изучить вопрос абсолютных путей к шаблонам `Зарезервировано: Спайд`
...
- [ ] Убедиться, что сессии не хранятся у клиентов и DTCJsonы не находятся в кукизах. `Свободно`
    - [ ] Если таки храняться, то сделать так, чтобы этого не было `Свободно`

### Рефакторинг
- [ ] Переделать систему наименования метадокументов о проекте.
