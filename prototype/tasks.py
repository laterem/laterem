...
class AnswerMatching(Task):
    def configure(self, correct_answers: List[str]) -> None:
...
class NumberNotationConvertation(Task):
    ...
    def test(self, answer: str) -> int:
        try:
            return int(str, self.destination_base) == self.source
...
class BasicProblemSolving(Task):
    ...
    def render(self) -> str:
        from custommath import int2str
        return f"{int2str(self.a, self.a_base)} по основанию {self.a_base} {'+' if self.op == BasicProblemSolving.Plus else '-'} {int2str(self.b, self.b_base)} по основанию {self.b_base} = ___"
